...
def _mc_ensure_debug_logging_locked(meshcore_log):
    ...
    if _mc_debug_raw_handler is None:
        handler = logging.FileHandler("/tmp/overmesh-meshcore-raw.log")
...
def enable_mc_debug(config_id, duration=60):
    ...
    /tmp/overmesh-meshcore-raw.log so we can see if packets arrive at all."""
    meshcore_log = logging.getLogger("meshcore")
    ...
    log.info(f"[MC:{config_id}] Debug event logging ON for {duration}s — raw bytes → /tmp/overmesh-meshcore-raw.log")
